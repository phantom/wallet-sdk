import { EmbeddedProvider, type EmbeddedProviderConfig } from './index';
...
describe('EmbeddedProvider Auth Flows', () => {
  ...
  let mockPhantomConnectAuth: jest.Mocked<PhantomConnectAuth>;
  let mockJWTAuth: jest.Mocked<JWTAuth>;
  ...
  beforeEach(() => {
    ...
    // Mock URL params accessor instead of window.location
    ...
    // Mock PhantomConnectAuth
    mockPhantomConnectAuth = {
      authenticate: jest.fn(),
      resumeAuthFromRedirect: jest.fn(),
    } as any;
    (PhantomConnectAuth as jest.Mock).mockImplementation(() => mockPhantomConnectAuth);
    PhantomConnectAuth.generateSessionId = jest.fn().mockReturnValue('test-session-id');

    // Mock JWTAuth
    mockJWTAuth = {
      authenticate: jest.fn(),
    } as any;
    (JWTAuth as jest.Mock).mockImplementation(() => mockJWTAuth);
  ...
  describe('Resume from Redirect Flow', () => {
    it('should resume connect from redirect with valid auth result', async () => {
      ...
      mockPhantomConnectAuth.resumeAuthFromRedirect.mockReturnValue(authResult);
  ...
  describe('Resume from Redirect - Invalid Session ID', () => {
    ...
    it('should start fresh authentication flow when session ID mismatch detected', async () => {
      ...
      expect(mockClient.createOrganization).toHaveBeenCalled();
      expect(mockPhantomConnectAuth.authenticate).toHaveBeenCalled();
    });
  });
  ...
  describe('App Wallet Flow', () => {
    ...
    it('should create app wallet directly without authentication', async () => {
      ...
      expect(mockPhantomConnectAuth.authenticate).not.toHaveBeenCalled();
      expect(mockJWTAuth.authenticate).not.toHaveBeenCalled();
    });
  ...
  describe('JWT Authentication Flow', () => {
    it('should authenticate with valid JWT token', async () => {
      ...
      mockStorage.getSession.mockResolvedValue(null);
      mockClient.createOrganization.mockResolvedValue({ organizationId: 'new-org-id' });
      mockJWTAuth.authenticate.mockResolvedValue({
    ...
    it('should handle JWT authentication failure gracefully', async () => {
      ...
      mockStorage.getSession.mockResolvedValue(null);
      mockClient.createOrganization.mockResolvedValue({ organizationId: 'new-org-id' });
      mockJWTAuth.authenticate.mockRejectedValue(new Error('Invalid JWT token'));
  ...
  describe('Error Handling', () => {
    ...
    it('should clean up state on authentication failures', async () => {
      ...
      // For this test, let JWT auth succeed but getWalletAddresses fail
      mockJWTAuth.authenticate.mockResolvedValue({
        walletId: 'jwt-wallet-123',
        provider: 'jwt',
        userInfo: { sub: 'user-123' },
