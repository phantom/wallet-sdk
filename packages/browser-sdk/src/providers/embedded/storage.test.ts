...
describe('IndexedDBStorage', () => {
  ...
  describe('generateKeypair', () => {
    it('should generate and encode keypair', async () => {
      ...
      (nacl.sign.keyPair as jest.Mock).mockReturnValue(mockKeypair);
      (bs58.encode as jest.Mock).mockImplementation((data) => {
        if (data === mockPublicKey) return 'encodedPublicKey';
        if (data === mockSecretKey) return 'encodedSecretKey';
        return '';
      });

      const keypair = await storage.generateKeypair();

      expect(nacl.sign.keyPair).toHaveBeenCalled();
      expect(bs58.encode).toHaveBeenCalledWith(mockPublicKey);
      expect(bs58.encode).toHaveBeenCalledWith(mockSecretKey);
  ...
  // Skipping error handling tests for fake-indexeddb implementation details
  ...
  describe('concurrent operations', () => {
    it('should handle concurrent session saves', async () => {
      ...
      expect(sessions.some(s => 
        s.walletId === retrievedSession!.walletId &&
        s.organizationId === retrievedSession!.organizationId
      )).toBe(true);
    });
    ...
    it('should handle concurrent reads', async () => {
      ...
      const results = await Promise.all(
        Array.from({ length: 10 }, () => storage.getSession())
      );
