...
describe('Browser Detection', () => {
  describe('parseBrowserFromUserAgent', () => {
    ...
    it('should detect Samsung Internet correctly', () => {
      const userAgent = 'Mozilla/5.0 (Linux; Android 13; SM-G998B) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/23.0 Chrome/115.0.0.0 Mobile Safari/537.36';
      const result = parseBrowserFromUserAgent(userAgent);
    ...
    it('should extract only major version number', () => {
      const userAgent = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.1.2.3 Safari/537.36';
      const result = parseBrowserFromUserAgent(userAgent);
      expect(result.name).toBe('chrome');
