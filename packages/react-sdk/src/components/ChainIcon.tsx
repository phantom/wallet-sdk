...
export function ChainIcon({ addressType, size = 8 }: ChainIconProps) {
  ...
  if (type.includes("solana")) {
    return <IconWrapper><Icon type="solana" size={size} color={theme.text} /></IconWrapper>;
  }

  if (type.includes("ethereum") || type.includes("evm")) {
    return <IconWrapper><Icon type="ethereum" size={size}  color={theme.text} /></IconWrapper>;
  }

  if (type.includes("bitcoin")) {
    return <IconWrapper><Icon type="bitcoin" size={size}  color={theme.text} /></IconWrapper>;
  }

  if (type.includes("sui")) {
    return <IconWrapper><Icon type="sui" size={size}  color={theme.text} /></IconWrapper>;
  }
