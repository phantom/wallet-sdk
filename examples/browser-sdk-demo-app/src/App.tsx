...
function App() {
  ...
  // Create SDK instance
  const createSDK = useCallback((type: ProviderType): BrowserSDK => {
    // Set debug config
    debug.enable()
  ...
  // Update balance
  const updateBalance = useCallback(async (addrs: WalletAddress[] = addresses) => {
    ...
    <div className="app">
      ...
      {isConnected && providerType === "injected" && (
        <section className="provider-section">
          ...
          <div className="provider-controls">
            <button onClick={handleEnableAutoConfirm}>
              Enable Auto-Confirm
            </button>
            <button onClick={handleDisableAutoConfirm}>
              Disable Auto-Confirm
            </button>
            <button onClick={handleGetAutoConfirmStatus}>
              Get Status
            </button>
            <button onClick={handleGetSupportedChains}>
              ...
              debugMessages.slice(-20).map((msg, index) => (
                <div key={index} className="debug-message">
                  <div className="debug-header">
                    ...
                    {DebugLevel[msg.level]} - {msg.category}
                  </div>
