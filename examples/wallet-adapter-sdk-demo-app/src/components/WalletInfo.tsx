        ...
        <div className="balance-row">
          <div className="balance-info">
            ...
            <span className="balance-value">
              {loading ? "Loading..." : error ? "Error" : balance !== null ? `${balance} SOL` : "--"}
            </span>
          </div>
          <button className="small" onClick={() => refetch()} disabled={loading}>
            {loading ? "Loading..." : "Refresh"}
          </button>
        </div>

        {error && <div className="error-text">Balance error: {error}</div>}
      </div>
    </section>
  );
}
